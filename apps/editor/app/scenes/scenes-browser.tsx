...
import { listLocalScenes } from '@/lib/local-scene-store'
...
/**
 * Client scenes browser used for static export (and as the body of the
 * server page when the API is unavailable). Reads the localStorage index.
 */
export function ScenesBrowser() {
  const [scenes, setScenes] = useState<SceneMeta[]>([])
  const server = isSceneServerAvailable()

  useEffect(() => {
    if (server) return
    setScenes(listLocalScenes())
  }, [server])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const list = Array.isArray(payload) ? payload : (payload.scenes ?? [])
        if (!cancelled) setScenes(list)
      } catch {
        if (!cancelled) setScenes(listLocalScenes())
      }
    })()
    return () => {
      cancelled = true
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-5xl px-6 py-12">
        ...
        <p className="mb-8 text-muted-foreground text-sm">
          {!server
            ? 'Static / embed mode — scenes stay in this browser (localStorage).'
            : scenes.length === 0
