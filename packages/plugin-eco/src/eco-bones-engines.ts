/**
 * Bones pure engines (computeLevel / computeTakeoff / FramingNode) — loaded
 * via dynamic `import()` when the construction panel needs them so the editor
 * pays nothing for electrical/HVAC/plumbing/framing tables on page open.
 *
 ...
 *
 * Jurisdiction: Ventura County is inlined in `bones-vendor/.../profiles.ts`
 * (no national jurisdictions-*.json on the default path).
 */
...
let enginesPromise: Promise<BonesEngines> | null = null

/**
 * Dynamic-import the vendored Bones closure (same idea as the world's glTF
 * loader). First call pays for the engines chunk; later calls reuse it.
 */
export function loadBonesEngines(): Promise<BonesEngines> {
  if (!enginesPromise) {
    enginesPromise = (async () => {
      const [computeMod, schemaMod, takeoffMod] = await Promise.all([
        import('./bones-vendor/src/framing/compute'),
        import('./bones-vendor/src/framing/schema'),
 ...
 * Awaits the engines chunk on first use.
 */
export async function runBonesMemberTakeoff(nodes: NodesRecord): Promise<{
  ...
  const { levelId } = pick
  try {
    const engines = await loadBonesEngines()
    const config = engines.FramingNode.parse({
      parentId: levelId,
      // Ventura County profile (inlined in vendor profiles.ts — no national JSON).
      jurisdiction: 'US-CA-VENTURA',
      detail: '400',
      studSpacingIn: 16,
      showWalls: true,
      showFloor: true,
